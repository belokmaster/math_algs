...
func generateMatrix(n, m int) [][]int {
	...
	for i := range matrix {
		...
		for j := range matrix[i] {
			matrix[i][j] = rand.Intn(2) // Генерируем значения 0 или 1 для логических операций
		}
	}
	return matrix
}

func logicalMultiplyMatrices(A, B [][]int) [][]int {
	n := len(A)
	m := len(B[0])
	p := len(B)
	...
	for i := 0; i < n; i++ {
		for j := 0; j < m; j++ {
			for k := 0; k < p; k++ {
				// Логическое умножение: 1, если оба числа ненулевые, иначе 0
				C[i][j] |= A[i][k] & B[k][j]
			}
		}
	}
...
func main() {
	...
	C := logicalMultiplyMatrices(A, B)

	fmt.Println("\nРезультат логического умножения A * B:")
	for _, row := range C {
		fmt.Println(row)
	}
